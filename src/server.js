...
// Migrate deprecated CLAWDBOT_* env vars → OPENCLAW_* so existing Northflank deployments
// keep working. Users should update their Northflank Variables to use the new names.
for (const suffix of ["PUBLIC_PORT", "STATE_DIR", "WORKSPACE_DIR", "GATEWAY_TOKEN", "CONFIG_PATH"]) {
  ...
  if (process.env[oldKey] && !process.env[newKey]) {
    ...
    // Best-effort compatibility shim for old Northflank templates.
    // Intentionally no warning: Northflank templates can still set legacy keys and warnings are noisy.
  }
...
// boot but the Northflank domain will be routed to a different port.
//
// OPENCLAW_PUBLIC_PORT is kept as an escape hatch for non-Northflank deployments.
...
// Debug breadcrumbs for common Northflank failures (502 / "Application failed to respond").
...
// Public health endpoint (no auth) so Northflank can probe without /setup.
...
<body>
  ...
  <div class="card">
    ...
    <p class="muted">Use this to configure an OpenAI-compatible API that requires a custom base URL (e.g. Ollama, vLLM, LM Studio, hosted proxies). You usually set the API key as a Northflank variable and reference it here.</p>
...
app.post("/setup/api/run", requireSetupAuth, async (req, res) => {
  ...
  if (ok) {
    ...
    // Northflank runs behind a reverse proxy. Trust loopback as a proxy hop so local client detection
...
app.post("/setup/api/reset", requireSetupAuth, async (_req, res) => {
  ...
  try {
    // Stop gateway to avoid running gateway + onboard concurrently on small Northflank instances.
    try {
      if (gatewayProc) {
        try { gatewayProc.kill("SIGTERM"); } catch {}
        await sleep(750);
...
const server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  // (e.g. Northflank variable update) but the config file still has the old value.
